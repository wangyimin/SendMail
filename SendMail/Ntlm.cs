...
namespace System.Net
{
    ...
    public class Ntlm
    {
        ...
        private static byte[] makeNtlm2Hash(string domain, string userName, string password)
        {
            ...
            MD4 md4 = new MD4();
            pwHash = md4.GetByteHashFromBytes(pwBytes);
        ...
        private unsafe byte[] ProcessChallengeMessage(byte[] blob)
        {
            ...
            if (targetInfo.Length > 0)
            {
                ...
                while (info.Length >= 4)
                {
                    ...
                    if (ID == 7) // Timestamp
                    {
                        ...
                        time = DateTime.FromFileTimeUtc(BitConverter.ToInt64(info.Slice(4, 8).ToArray(),0));
                    }
            ...
            new RNGCryptoServiceProvider().GetBytes(exportedSessionKey);
            ...
            Span<byte> negotiateBuffer = new byte[sizeof(NegotiateMessage)];
            ...
            hmac.AppendData(negotiateBuffer.ToArray());
            hmac.AppendData(blob);
            //WANG
            //hmac.AppendData(responseBytes.AsSpan(0, payloadOffset));
            hmac.AppendData(responseBytes.AsSpan(0, payloadOffset).ToArray());
